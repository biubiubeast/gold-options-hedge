      ...
      <div ref={viewportRef} className="min-w-0 flex-1 overflow-auto">
        <div style={{ display: "grid", gridTemplateColumns: template, minWidth } as CSSProperties}>
          ...
          {columnValues.map(column => {
            ...
            const label = <button type="button" data-expiry-header={!transpose ? String(column) : undefined} aria-label={!transpose ? `Expiry ${String(column)} · DTE ${expiryDte ?? "MISSING"} days summary` : undefined} data-spot-column={isSpotColumn ? "true" : undefined} className={`sticky top-0 z-20 flex h-8 flex-col items-center justify-center truncate border-b border-r border-border/40 bg-background px-0.5 font-mono text-[8px] leading-none ${isSpotColumn ? "border-x-amber-300/70 text-amber-300" : "text-foreground/75"}`} title={transpose ? `Strike ${formatPrice(strikeColumn)} · ${formatStrikeDistanceFromSpot(strikeColumn!, spot)} vs spot` : `${String(column)} · DTE ${expiryDte ?? "MISSING"} days`}>{transpose ? <>{formatPrice(strikeColumn)}{isAtmColumn ? " · ATM" : ""}</> : <><span>{String(column).slice(5)}</span><strong className="mt-0.5 text-[7px] text-amber-300">DTE {expiryDte ?? "?"}</strong></>}</button>;
          ...
          {rowValues.flatMap(row => {
            ...
            const axisLabel = <button type="button" data-expiry-header={transpose ? String(row) : undefined} aria-label={transpose ? `Expiry ${String(row)} · DTE ${rowExpiryDte ?? "MISSING"} days summary` : undefined} data-spot-row={isSpotRow ? "true" : undefined} title={rowStrike === null ? `${String(row)} · DTE ${rowExpiryDte ?? "MISSING"} days` : `Strike ${formatPrice(rowStrike)} · ${strikeDistance} vs spot`} style={{ height: rowHeight }} className={`sticky left-0 z-10 flex items-center justify-between overflow-hidden border-b border-r bg-background px-1 font-mono text-[8px] ${isSpotRow ? "border-y-amber-300/80 bg-amber-400/10 text-amber-300" : "border-border/40 text-foreground/75"}`}><span>{transpose ? String(row).slice(5) : formatPrice(rowStrike)}</span>{transpose ? <strong className="text-[7px] text-amber-300">DTE {rowExpiryDte ?? "?"}</strong> : strikeDistance !== null && <span className="text-[7px]">{strikeDistance}</span>}</button>;
