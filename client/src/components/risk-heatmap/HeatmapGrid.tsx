...
const heldStatusAbbreviation = (status: ReturnType<typeof worstStatus>) => ({ LIVE: "L", STALE: "S", WARN: "W", MISSING: "M", FAIL: "F" })[status];
...
const ivWithSource = (value: number | null, metric: "bidIV" | "askIV", derived?: boolean) => `${formatCompact(value, metric)}${derived && value !== null ? " CALC" : ""}`;
...
function TooltipPosition({ position, metric, preset, content }: { position: EnrichedRiskPosition; metric: HeatmapMetric; preset: HoverDataPreset; content: Props["hoverContent"] }) {
  ...
  if (preset === "market" || preset === "all") {
    ...
    if (content.bidAskIv) rows.push(["Bid IV / Ask IV", `${ivWithSource(position.bidIV, "bidIV", position.bidIvDerived)} / ${ivWithSource(position.askIV, "askIV", position.askIvDerived)}`]);
...
export function HeatmapGrid({ cells, expiries, strikes, metric, scale, importanceCutoff, lowImportanceCutoff, transpose, reverseStrikes, cellSize, fitAll, spot, callPut, moneyness, atmStrike, highlightCellKey, labelMode, hoverPreset, sequentialMagnitude, heldCellContent, hoverContent, cellDetailEnabled, expiryDetailEnabled, targetMode, targetCellKeys, onTargetCellChange, onSelectPosition, onSelectExpiry }: Props) {
  ...
  const atmStrikes = useMemo(() => new Set(atmStrike === undefined
    ? nearestStrikeLevels(strikes, spot)
    : atmStrike === null ? [] : [atmStrike]), [atmStrike, spot, strikes]);
  const displayStrikes = useMemo(() => [...strikes].sort((a, b) => reverseStrikes ? b - a : a - b), [reverseStrikes, strikes]);
  const cellMap = useMemo(() => new Map(cells.map(cell => [cell.key, cell])), [cells]);
  const scopePositions = useMemo(() => cells.flatMap(cell => cell.positions), [cells]);
    ...
    <div className="relative flex min-h-0 flex-1 overflow-hidden border border-border/60 bg-background/70" data-testid="risk-heatmap-grid">
      <div ref={viewportRef} className="min-w-0 flex-1 overflow-auto">
        <div style={{ display: "grid", gridTemplateColumns: template, minWidth } as CSSProperties}>
          <HeatmapScopeTooltip positions={scopePositions} metric={metric} moneyness={moneyness} label={transpose ? "EXP / STRIKE" : "STRIKE / EXP"} />
          {columnValues.map(column => {
            ...
            const expiryDte = !transpose ? expiryDteByExpiry.get(String(column)) ?? null : null;
            const label = <button type="button" data-expiry-header={!transpose ? String(column) : undefined} aria-label={!transpose ? `Expiry ${String(column)} · DTE ${expiryDte ?? "MISSING"} days summary` : undefined} data-spot-column={isSpotColumn ? "true" : undefined} className={`sticky top-0 z-20 flex h-8 flex-col items-center justify-center truncate border-b border-r border-border/40 bg-background px-0.5 font-mono text-[8px] leading-none ${isSpotColumn ? "border-x-amber-300/70 text-amber-300" : "text-foreground/75"}`} title={transpose ? `Strike ${formatStrike(strikeColumn)} · ${formatStrikeDistanceFromSpot(strikeColumn!, spot)} vs spot` : `${String(column)} · DTE ${expiryDte ?? "MISSING"} days`}>{transpose ? <>{formatStrike(strikeColumn)}{isAtmColumn ? " · ATM" : ""}</> : <><span>{String(column).slice(5)}</span><strong className="mt-0.5 text-[7px] text-amber-300">DTE {expiryDte ?? "?"}</strong></>}</button>;
            return transpose ? <div key={String(column)} className="contents">{isSpotColumn ? <SpotPriceTooltip spot={spot}>{label}</SpotPriceTooltip> : label}</div> : <ExpiryTooltip key={String(column)} expiry={String(column)} positions={cells.filter(cell => cell.expiry === String(column)).flatMap(cell => cell.positions)} metric={metric} detailEnabled={expiryDetailEnabled} onSelectExpiry={onSelectExpiry}>{label}</ExpiryTooltip>;
          })}
          {rowValues.flatMap(row => {
            ...
            const rowExpiryDte = transpose ? expiryDteByExpiry.get(String(row)) ?? null : null;
            const axisLabel = <button type="button" data-expiry-header={transpose ? String(row) : undefined} aria-label={transpose ? `Expiry ${String(row)} · DTE ${rowExpiryDte ?? "MISSING"} days summary` : undefined} data-spot-row={isSpotRow ? "true" : undefined} title={rowStrike === null ? `${String(row)} · DTE ${rowExpiryDte ?? "MISSING"} days` : `Strike ${formatStrike(rowStrike)} · ${strikeDistance} vs spot`} style={{ height: rowHeight }} className={`sticky left-0 z-10 flex items-center justify-between overflow-hidden border-b border-r bg-background px-1 font-mono text-[8px] ${isSpotRow ? "border-y-amber-300/80 bg-amber-400/10 text-amber-300" : "border-border/40 text-foreground/75"}`}><span>{transpose ? String(row).slice(5) : formatStrike(rowStrike)}</span>{transpose ? <strong className="text-[7px] text-amber-300">DTE {rowExpiryDte ?? "?"}</strong> : strikeDistance !== null && <span className="text-[7px]">{strikeDistance}</span>}</button>;
            const axis = transpose ? <ExpiryTooltip key={`axis-${String(row)}`} expiry={String(row)} positions={cells.filter(cell => cell.expiry === String(row)).flatMap(cell => cell.positions)} metric={metric} detailEnabled={expiryDetailEnabled} onSelectExpiry={onSelectExpiry}>{axisLabel}</ExpiryTooltip> : <div key={`axis-${String(row)}`} className="contents">{isSpotRow ? <SpotPriceTooltip spot={spot}>{axisLabel}</SpotPriceTooltip> : axisLabel}</div>;
            const buttons = columnValues.map(column => {
              ...
              const heldCallPutMarker = heldCallPuts.size > 1 ? "C/P" : heldCallPuts.has("call") ? "C" : heldCallPuts.has("put") ? "P" : null;
              const heldStatusMarker = cell?.held ? heldStatusAbbreviation(status) : null;
              const heldMarkerVisible = Boolean(cell?.held && (
              ...
              return <Tooltip key={key} delayDuration={80} open={hoveredCellKey === key} onOpenChange={open => setHoveredCellKey(open ? key : null)}><TooltipTrigger asChild><button
                type="button" data-cell-key={key} data-held={cell?.held ? "true" : "false"} data-xaut-held={xautHeld ? "true" : "false"} data-target-selected={targetSelected ? "true" : "false"}
                ...
                onClick={() => {
                  ...
                  if (cellDetailEnabled && topPositions[0]) onSelectPosition(topPositions[0]);
                }}
      ...
      <aside className="flex w-[62px] shrink-0 flex-col items-center border-l border-border/60 bg-card/50 px-1 py-2" aria-label="vertical heatmap legend">
        ...
        {targetCellKeys.size > 0 && <span className="mt-1 text-center text-[6px] font-semibold leading-tight text-sky-300">SKY BLUE<br />TARGET</span>}
      </aside>
    </div>
  );
}
