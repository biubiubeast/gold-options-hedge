...
function ExpiryTooltip({ expiry, positions, metric, detailEnabled, onSelectExpiry, children }: { expiry: string; positions: EnrichedRiskPosition[]; metric: HeatmapMetric; detailEnabled: boolean; onSelectExpiry: Props["onSelectExpiry"]; children: ReactElement<{ onClick?: MouseEventHandler }> }) {
  ...
  const dte = positions.find(position => Number.isFinite(position.dte))?.dte ?? null;
    ...
    <div className="relative flex min-h-0 flex-1 overflow-hidden border border-border/60 bg-background/70" data-testid="risk-heatmap-grid">
      <div ref={viewportRef} className="min-w-0 flex-1 overflow-auto">
        ...
        <div style={{ display: "grid", gridTemplateColumns: template, minWidth } as CSSProperties}>
          <HeatmapScopeTooltip positions={scopePositions} metric={metric} label={transpose ? "EXP / STRIKE" : "STRIKE / EXP"} />
