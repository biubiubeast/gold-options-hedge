...
function freshness(value?: SpotPrice | null) {
  ...
  if (value.status === "realtime")
    return { label: "实时", className: "bg-emerald-400" };
  if (value.status === "stale" || value.stale)
    return { label: "旧值/休市", className: "bg-amber-500" };
...
function SpotChip({
  ...
  return (
    ...
    >
      <span
        className={`h-1.5 w-1.5 rounded-full ${state.className}`}
        aria-hidden="true"
      />
      <span className="text-[10px] font-medium text-muted-foreground sm:text-xs">
        {label}
      </span>
      <strong className="font-mono text-xs font-semibold tabular-nums sm:text-sm">
        ${price(value?.price)}
      </strong>
...
export function LiveSpotBar() {
  ...
  const { data, isFetching, refetch } = trpc.market.spotPrices.useQuery(
    undefined,
  ...
  return (
    <div className="flex items-center gap-1.5" aria-label="实时现价">
      <SpotChip label="XAUT/USDT" value={data?.xaut} timeZone={timeZone} />
      <SpotChip label="GLD/USD" value={data?.gld} timeZone={timeZone} />
      <SpotChip label="BTC/USDT" value={data?.btc} timeZone={timeZone} />
      <SpotChip label="ETH/USDT" value={data?.eth} timeZone={timeZone} />
      <Button
        type="button"
        variant="ghost"
        size="icon"
