...
const iv = (value: number | null | undefined, model = false) =>
  value !== null && value !== undefined && Number.isFinite(value)
    ? `${(value * 100).toFixed(2)}%${model ? " MODEL" : ""}`
    : "—";
...
export default function OptionDetail() {
  ...
  const { data: position, isLoading } = trpc.positions.get.useQuery(
    { id: positionId },
  ...
  const marketRows = [
    ...
    ["Market Mark IV", market.markIv > 0 ? iv(market.markIv) : "—"],
    ["Market Bid / Ask IV", `${iv(market.bidIv)} / ${iv(market.askIv)}`],
    ["Model Mark IV", iv(market.modelMarkIv, true)],
    [
      "Model Bid / Ask IV",
      `${iv(market.modelBidIv, true)} / ${iv(market.modelAskIv, true)}`,
    ],
    ["Model Bid Ask IV Spread", iv(market.modelIvSpread, true)],
    [
      "IV Reference Spot",
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        <Button variant="ghost" size="icon" onClick={() => navigate("/matrix")}>
        ...
        <div className="flex-1">
          ...
          <p className="text-sm text-muted-foreground mt-1">
            {position.underlying} {position.strike}{" "}
            {position.optionType.toUpperCase()} · {position.expiry}
          </p>
        </div>
      </div>
        ...
        <Card className="glass-card">
          ...
          <CardContent className="space-y-3">
            {[
              ...
              ["Type", position.optionType.toUpperCase()],
              ["Strike", position.strike],
              ["Expiry", position.expiry],
              ["Quantity", position.quantity],
              ["Entry Price", position.entryPrice],
              ["Fee", position.fee],
              ["Entry Delta", position.entryDelta],
              ["Contract Multiplier", calculated.contractMultiplier.toString()],
            ].map(([label, value]) => (
      ...
      <Card className="glass-card">
        ...
        <CardContent className="grid grid-cols-2 md:grid-cols-4 gap-6">
          <div>
            <p className="text-xs text-muted-foreground uppercase">Delta (Δ)</p>
            <p className="text-lg font-semibold mt-1 font-mono">
              {market.delta.toFixed(6)}
            </p>
            <p className="text-xs text-primary mt-1">
              Raw Total: {calculated.totalDeltaRaw.toFixed(4)}
            </p>
            <p className="text-xs text-muted-foreground">
              XAU Total: {calculated.totalDeltaXau.toFixed(4)}
            </p>
          </div>
          <div>
            <p className="text-xs text-muted-foreground uppercase">Gamma (Γ)</p>
            <p className="text-lg font-semibold mt-1 font-mono">
              {market.gamma.toFixed(8)}
            </p>
            <p className="text-xs text-primary mt-1">
              Raw Total: {calculated.totalGammaRaw.toFixed(6)}
            </p>
            <p className="text-xs text-muted-foreground">
              XAU Total: {calculated.totalGammaXau.toFixed(6)}
            </p>
          </div>
          <div>
            ...
            <p className="text-lg font-semibold mt-1 font-mono">
              {market.theta.toFixed(6)}
            </p>
            <p className="text-xs text-primary mt-1">
              Total: {money(calculated.totalTheta)}
            </p>
          </div>
          <div>
            <p className="text-xs text-muted-foreground uppercase">
            ...
            <p className="text-lg font-semibold mt-1 font-mono">
              {market.vega.toFixed(6)}
            </p>
            <p className="text-xs text-primary mt-1">
              Total: {money(calculated.totalVega)}
            </p>
          </div>
        </CardContent>
      </Card>
      ...
      <Card className="glass-card">
        ...
        <CardContent className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div>
            ...
            <p className="text-xl font-semibold mt-1">
              {money(calculated.currentValue)}
            </p>
          </div>
          <div>
            <p className="text-xs text-muted-foreground uppercase">P&L</p>
            <p
              className={`text-xl font-semibold mt-1 ${calculated.pnl >= 0 ? "text-green-400" : "text-red-400"}`}
            >
              {calculated.pnl >= 0 ? "+" : ""}
              {money(calculated.pnl)}
            </p>
          </div>
        </CardContent>
      </Card>
