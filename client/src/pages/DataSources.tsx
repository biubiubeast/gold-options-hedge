...
export default function DataSources() {
  ...
  const {
    ...
    refetch,
    isFetching,
  } = trpc.market.spotPrices.useQuery(undefined, MARKET_QUERY_OPTIONS);
    ...
    <div className="space-y-6">
          ...
          <Card key={source.product} className="glass-card">
            <CardHeader className="pb-2">
              <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                <CardTitle className="text-base">{source.product}</CardTitle>
                <Badge variant="outline">{source.mode}</Badge>
              </div>
            </CardHeader>
            <CardContent className="space-y-3 text-sm">
              <div className="grid grid-cols-1 md:grid-cols-[10rem_1fr] gap-1 md:gap-3">
                <span className="text-muted-foreground">Provider</span>
                <span>{source.provider}</span>
                <span className="text-muted-foreground">Endpoint</span>
                <code className="text-xs break-all">{source.endpoint}</code>
              ...
              >
                官方/来源页面 <ExternalLink className="w-3.5 h-3.5" />
              </a>
            </CardContent>
          </Card>
        ))}
      ...
      <Card className="glass-card border-primary/20">
        ...
        <CardContent className="space-y-4 text-sm">
          <div className="grid grid-cols-1 gap-1 md:grid-cols-[10rem_1fr] md:gap-3">
            <span className="text-muted-foreground">历史逐 Strike OI</span>
            <code className="break-all text-xs">
              {maxPainCoverage?.primaryEndpoint ?? "SignalPlus mizar gateway"}
            </code>
            <span className="text-muted-foreground">备用 OI 端点</span>
            <code className="break-all text-xs">
              {maxPainCoverage?.mirrorEndpoint ?? "signalplus.net mirror"}
            </code>
            <span className="text-muted-foreground">BTC K线优先级</span>
            <span>Coinbase Exchange → OKX Spot → Binance Spot</span>
            <span className="text-muted-foreground">查询边界</span>
            <span>
              {maxPainCoverage
                ? `${maxPainCoverage.earliestVerifiedDate} 起实测；单次最多 ${maxPainCoverage.maximumDaysPerCalculation} 天；UTC ${maxPainCoverage.observationHoursUtc.map(hour => String(hour).padStart(2, "0")).join("/")}`
                : "正在读取服务端配置"}
            </span>
          </div>
          ...
          <div className="flex flex-wrap gap-4">
            <a
              href="https://insights.deribit.com/dev-hub/deribit-max-pain-python-code/"
              target="_blank"
              rel="noreferrer"
              className="inline-flex items-center gap-1.5 text-primary hover:underline"
            >
            ...
            <a
              ...
              className="inline-flex items-center gap-1.5 text-primary hover:underline"
            >
              Coinbase Candles <ExternalLink className="h-3.5 w-3.5" />
            </a>
            <a
              href="https://www.okx.com/docs-v5/en/#rest-api-market-data-get-candlesticks-history"
              target="_blank"
              rel="noreferrer"
              className="inline-flex items-center gap-1.5 text-primary hover:underline"
            >
              OKX History Candles <ExternalLink className="h-3.5 w-3.5" />
            </a>
          </div>
        </CardContent>
      </Card>
      ...
      <Card
        className={
          data?.marketDataConfigured || data?.tradierConfigured
      ...
      >
        <CardContent className="p-4 text-sm leading-relaxed">
          <p className="font-medium">
            当前 GLD 优先级：{data?.gldPriority || "检测中"}
          </p>
          <p className="text-muted-foreground mt-1">
            推荐在服务器设置 <code>MARKETDATA_TOKEN</code> 并完成 OPRA
            实时权限，可获得 GLD 逐合约低延迟 Bid/Ask、IV 与
            Delta/Gamma/Theta/Vega；备用的 Tradier production token
            可提供实时报价，但其 ORATS Greeks
            官方标注约每小时更新。两者都未配置时，现价使用 Yahoo fallback，期权
            Greeks 使用“公式管理”中的 Black-Scholes 估算。
          </p>
        </CardContent>
      </Card>
    </div>
