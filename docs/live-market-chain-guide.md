...
## 2. GLD FULL CHAIN 热力图

矩阵页 DATA 默认选择 `GLD FULL CHAIN`。当前全链来自 Cboe 的 GLD delayed-options 文件，包含所有返回的 Expiry、Strike、Call/Put、Bid/Ask、IV、Greeks、Open Interest 与 Volume。
...
- Metric 可切换 Unit Delta、Total Delta、Mark IV 等；上市未持仓合约的 Total 指标显示 MISSING，不伪造为 0。
...
- Cboe GLD 全链：无需密钥，延迟行情；页面明确显示 As-of、分钟数和 STALE；
