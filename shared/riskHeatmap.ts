...
export interface RiskPosition {
  ...
  bidSize: number | null;
  askSize: number | null;
  /** Market/provider-published IV fields. Never populated by our inversion model. */
  markIV: number | null;
  bidIV: number | null;
  askIV: number | null;
  ivSpread: number | null;
  /** Website valuation-model IV fields, always labelled MODEL in the UI. */
  ...
  /** Underlying reference synchronized to the option-price snapshot. */
...
export function percentile(values: number[], p: number): number {
  ...
  const index = Math.max(
    0,
    Math.min(sorted.length - 1, (sorted.length - 1) * p)
  );
...
export function expiryBucketMatchesDte(
  dte: number,
  bucket: ExpiryBucket
): boolean {
...
export function positionLabel(
  position: Pick<RiskPosition, "underlying" | "expiry" | "strike" | "callPut">
): string {
...
export function metricValue(
  ...
  switch (metric) {
    case "unitDelta":
      return position.unitDelta;
    case "totalDelta":
      return position.totalDeltaXAU;
    case "gamma":
      return position.totalGammaXAU;
    case "theta":
      return position.totalThetaUSD;
    case "vega":
      return position.totalVegaUSD;
    case "markIV":
      return position.markIV;
    case "bidIV":
      return position.bidIV;
    case "askIV":
      return position.askIV;
    case "ivSpread":
      return position.ivSpread;
    case "modelMarkIV":
      return position.modelMarkIV;
    case "modelBidIV":
      return position.modelBidIV;
    case "modelAskIV":
    ...
    case "notionalSize":
      return position.notionalSizeUSD;
    case "bidDollarNotional":
      return position.bidDollarNotional;
    case "askDollarNotional":
      return position.askDollarNotional;
    case "bidAskDollarNotional":
      return position.bidAskDollarNotional;
    case "MV":
      return position.MV;
    case "UPL":
      return position.UPL;
    case "DTE":
      return Number.isFinite(position.dte) ? position.dte : null;
    case "distanceToStrike":
      return position.distanceToStrike;
    case "rollPriority":
      return position.rollPriority.total;
  }
}
...
export function aggregateHeatmapCellMetric(
  positions: EnrichedRiskPosition[],
  metric: HeatmapMetric
): number | null {
  ...
  if (
    metric === "unitDelta" &&
    eligible.some(position => metricValue(position, metric) === 0)
  )
    return null;
...
export function expiryHeldMetricTotal(
  ...
  return {
    label,
    value: (values as number[]).reduce((total, value) => total + value, 0),
  };
}
...
export function spotRangeState(
  strikes: number[],
  spot: number
): SpotRangeState {
  const valid = strikes.filter(Number.isFinite).sort((a, b) => a - b);
  if (valid.length === 0 || !Number.isFinite(spot) || spot <= 0)
    return { state: "missing", nearestStrike: null };
  if (spot < valid[0]) return { state: "below", nearestStrike: valid[0] };
  if (spot > valid[valid.length - 1])
    return { state: "above", nearestStrike: valid[valid.length - 1] };
  return {
    state: "within",
    nearestStrike: valid.reduce(
      (nearest, strike) =>
...
export function strikeDistanceFromSpot(
  strike: number,
  spot: number
): number | null {
...
export function formatStrikeDistanceFromSpot(
  strike: number,
  spot: number
): string {
...
export function nearestStrikeLevels(
  strikes: number[],
  spot: number,
  count = 1
): number[] {
...
export function worstStatus(
  positions: Pick<RiskPosition, "dataStatus">[]
): DataStatus {
  return positions.reduce<DataStatus>(
    (worst, position) =>
      statusSeverity(position.dataStatus) > statusSeverity(worst)
        ? position.dataStatus
        : worst,
    "LIVE"
  );
...
export function deriveDataStatus(
  ...
  if (
    position.contractAdjusted ||
    position.deliverableSource?.includes("fallback")
  )
    return "WARN";
...
export function deriveTotals(position: RiskPosition): RiskPosition {
  ...
  const marketValue =
    mark !== null && multiplier !== null && qty !== null
      ? mark * qty * multiplier
      : null;
  const totalDelta =
    canScale && position.unitDelta !== null
      ? position.unitDelta * qty * multiplier * ounces
      : null;
  const totalGamma =
    canScale && position.unitGamma !== null
      ? position.unitGamma * qty * multiplier * ounces ** 2
      : null;
  const totalTheta =
    multiplier !== null && qty !== null && position.unitTheta !== null
      ? position.unitTheta * qty * multiplier
      : null;
  const totalVega =
    multiplier !== null && qty !== null && position.unitVega !== null
  ...
  return {
    ...
    MV: marketValue,
    UPL:
      marketValue !== null && entryCost !== null
        ? marketValue - entryCost
        : null,
  };
}
...
function rollFactors(
  position: RiskPosition,
  spot: number,
  context: { portfolioAbsDelta: number; portfolioAbsMV: number },
  asOf: Date
): RollPriority {
  ...
  const thetaRatio =
    position.totalThetaUSD === null
      ? null
      : Math.abs(position.totalThetaUSD) / Math.max(mv, 1);
  const spread =
    position.bid !== null &&
    position.ask !== null &&
    position.markPrice &&
  ...
  const intrinsic =
    spot > 0
      ? Math.max(
          position.callPut === "call"
            ? spot - position.strike
            : position.strike - spot,
          0
        )
      : null;
  const timeValueRatio =
    position.markPrice !== null && position.markPrice > 0 && intrinsic !== null
      ? clamp01(
          Math.max(position.markPrice - intrinsic, 0) / position.markPrice
        )
      : null;
  const absDelta = Math.abs(position.totalDeltaXAU ?? 0);
  const deltaScore = Math.max(
    Math.abs(position.unitDelta ?? 0),
    clamp01(absDelta / 100)
  );
  const residualScale =
    context.portfolioAbsMV > 0 && spot > 0
  ...
  const factorInputs = [
    {
      ...
      label: "DTE",
      weight: 0.2,
      value: Number.isFinite(dte) ? clamp01(1 - Math.max(dte, 0) / 45) : null,
      reason: Number.isFinite(dte) ? `${dte} DTE` : "expiry missing",
    },
    {
      key: "thetaMv",
      label: "Theta / MV",
      weight: 0.15,
      value: thetaRatio === null ? null : clamp01(thetaRatio / 0.05),
      reason:
        thetaRatio === null
          ? "theta or MV missing"
          : `${(thetaRatio * 100).toFixed(2)}% / day`,
    },
    {
      key: "distance",
      label: "Distance to strike",
      weight: 0.15,
      value: distance === null ? null : clamp01(1 - Math.abs(distance) / 0.15),
      reason:
        distance === null
          ? "spot missing"
          : `${(distance * 100).toFixed(2)}% from strike`,
    },
    {
      key: "delta",
      label: "Unit / Total Delta",
      weight: 0.1,
      value:
        position.unitDelta === null && position.totalDeltaXAU === null
          ? null
          : clamp01(deltaScore),
      reason:
        position.totalDeltaXAU === null
          ? "delta missing"
          : `${formatCompact(position.totalDeltaXAU)} XAU delta`,
    },
    {
      key: "spread",
      label: "Liquidity spread",
      weight: 0.1,
      value: spread === null ? null : clamp01(spread / 0.2),
      reason:
        spread === null
          ? "bid/ask missing"
          : `${(spread * 100).toFixed(2)}% spread`,
    },
    {
      key: "timeValue",
      label: "Remaining time value",
      weight: 0.1,
      value: timeValueRatio === null ? null : 1 - timeValueRatio,
      reason:
        timeValueRatio === null
          ? "time value missing"
          : `${(timeValueRatio * 100).toFixed(1)}% time value`,
    },
    {
      key: "hedge",
      ...
      weight: 0.1,
      value:
        context.portfolioAbsDelta > 0
          ? clamp01(absDelta / context.portfolioAbsDelta / 0.25)
          : 0,
      reason: `${formatCompact(absDelta)} / ${formatCompact(context.portfolioAbsDelta)} abs XAU delta`,
    },
    {
      key: "residual",
      label: "Van residual improvement",
      weight: 0.1,
      value: residualScale,
      reason: `${(residualScale * 100).toFixed(0)}% residual-improvement proxy`,
    },
  ];
  ...
  return {
    total: factors.reduce((sum, factor) => sum + factor.contribution, 0),
    factors,
  };
}
...
export function enrichRiskPositions(
  ...
  const context = {
    portfolioAbsDelta: totaled.reduce(
      (sum, position) => sum + Math.abs(position.totalDeltaXAU ?? 0),
      0
    ),
    portfolioAbsMV: totaled.reduce(
      (sum, position) => sum + Math.abs(position.MV ?? 0),
      0
    ),
  };
  const modelFormulaNames = new Set<string>([
  ...
  const defaultModelExpressions = new Map<string, string>(
    DEFAULT_FORMULAS.map(formula => [formula.name, formula.defaultExpression])
  );
  const customModelFormula = formulas.some(
    formula =>
      modelFormulaNames.has(formula.name) &&
      formula.expression !== defaultModelExpressions.get(formula.name)
  );
  return totaled.map(position => {
    ...
    const ivReferenceSpot = finiteOrNull(position.ivReferenceSpot);
    const referenceAsOf = position.ivReferenceTime
      ? new Date(position.ivReferenceTime).getTime()
      : position.quoteTime
    ...
    const deriveModelIv = (priceValue: number | null) =>
      modelImpliedVolatilityFromPrice({
        ...
        asOf: Number.isFinite(referenceAsOf) ? referenceAsOf : Number.NaN,
        rate: options.riskFreeRate,
        formulas,
      });
    const disabledResult = { value: null, status: "DISABLED" as const };
    const chooseModelIv = (
      upstreamValue: number | null,
      upstreamStatus: ModelIvStatus,
      priceValue: number | null
    ) => {
      if (!inversionEnabled) return disabledResult;
      if (
        !customModelFormula &&
        upstreamValue !== null &&
        upstreamStatus === "MODEL"
      ) {
        return { value: upstreamValue, status: "MODEL" as const };
      }
      return deriveModelIv(priceValue);
    };
    const modelMark = chooseModelIv(
      finiteOrNull(position.modelMarkIV),
      position.modelMarkIvStatus,
      finiteOrNull(position.markPrice)
    );
    const modelBid = chooseModelIv(
      finiteOrNull(position.modelBidIV),
      position.modelBidIvStatus,
      bidPrice
    );
    const modelAsk = chooseModelIv(
      finiteOrNull(position.modelAskIV),
      position.modelAskIvStatus,
      askPrice
    );
    const modelMarkIV = modelMark.value;
    ...
    const bidAskDollarNotional =
      bidDollarNotional !== null && askDollarNotional !== null
        ? editableFormula(
            "bid_ask_dollar_notional",
            { bidDollarNotional, askDollarNotional },
            formulas,
            bidDollarNotional + askDollarNotional
          )
        : null;
    const enrichedBase: RiskPosition = {
      ...position,
      // Market IV remains exactly as published by the venue/provider. MODEL
      // IV is a separate value family and always uses the synchronized
      // IV Reference Spot, never the newer Display Spot used for ATM.
      markIV: finiteOrNull(position.markIV),
      bidIV: finiteOrNull(position.bidIV),
      askIV: finiteOrNull(position.askIV),
      ivSpread:
        finiteOrNull(position.bidIV) !== null &&
        finiteOrNull(position.askIV) !== null
          ? finiteOrNull(position.askIV)! - finiteOrNull(position.bidIV)!
          : null,
      modelMarkIV,
      modelBidIV,
      modelAskIV,
      modelIVSpread:
        modelBidIV !== null && modelAskIV !== null
          ? modelAskIV - modelBidIV
          : null,
      modelMarkIvStatus: modelMark.status,
      modelBidIvStatus: modelBid.status,
      modelAskIvStatus: modelAsk.status,
      ivReferenceSpot,
    ...
    return {
      ...
      notionalSizeUSD:
        position.positionKind === "listed"
          ? 0
          : (finiteOrNull(position.notionalSizeUSD) ??
      ...
      intrinsicValue,
      timeValue:
        position.markPrice !== null && intrinsicValue !== null
          ? Math.max(position.markPrice - intrinsicValue, 0)
          : null,
...
export function buildHeatScale(
  ...
  if (customMin !== null && customMax !== null && customMax >= customMin) {
    ...
    const boundaries = Array.from(
      { length: 7 },
      (_, index) => customMin + (spread * index) / 6
    );
    ...
    return {
      ...
      bins: boundaries.slice(0, -1).map((from, index) => {
        ...
        return {
          ...
          normalized: normalize((from + to) / 2),
        };
      }),
      normalize,
    };
  ...
  const normalize = (input: number): number => {
    ...
    if (mode === "log") {
      const magnitude =
        Math.log1p(Math.abs(input) / logScale) / Math.log1p(p99Abs / logScale);
...
export function formatStrike(value: number | null): string {
  if (value === null || !Number.isFinite(value)) return "MISSING";
  return canonicalStrike(value).toLocaleString("en-US", {
...
export function heatColor(normalized: number, centered: boolean): string {
  return magnitudeHeatColor(
    centered ? (Math.max(-1, Math.min(1, normalized)) + 1) / 2 : normalized
  );
}
...
export function magnitudeHeatColor(normalized: number): string {
  ...
  const lower =
    stops[Math.max(0, (upperIndex < 0 ? stops.length - 1 : upperIndex) - 1)];
  const ratio =
...
export function exerciseControl(
  ...
  const funding =
    multiplier !== null
      ? position.strike * multiplier * Math.abs(position.netQty)
      : null;
  const available = position.buyingPower ?? position.availableUSD ?? null;
  const coverage =
    funding !== null && funding > 0 && available !== null
  ...
  const status: DataStatus =
    likelyITM && coverage !== null && coverage < 100
      ? "FAIL"
...
export function calculateScenario(
  ...
  for (const position of positions) {
    ...
    const theoretical =
      remainingDays <= 0
        ? {
            price: Math.max(
              position.callPut === "call"
                ? shockedSpot - position.strike
                : position.strike - shockedSpot,
              0
            ),
            delta:
              position.callPut === "call"
    ...
    const pnl =
      (theoretical.price - position.markPrice) * position.netQty * multiplier;
    ...
    stressedDeltaXAU +=
      theoretical.delta * position.netQty * multiplier * ounces;
  }
  const optionPnlUSD = gldPnlUSD + xautPnlUSD + btcPnlUSD + ethPnlUSD;
  const vanNakedPnlUSD =
    (input.vanNakedDeltaXau * input.spots.XAU * input.xauShockPct) / 100;
  const residualPnlUSD = optionPnlUSD + vanNakedPnlUSD;
  const stressCoveragePct =
    vanNakedPnlUSD < 0 && optionPnlUSD > 0
...
export function generateMockPositions(
  ...
  for (let index = 0; index < count; index += 1) {
    ...
    const strike =
      Math.round((spot + strikeIndex * strikeStep) / strikeStep) * strikeStep;
    ...
    const markIV =
      (underlying === "GLD" ? 0.18 : 0.42) +
      random() * (underlying === "GLD" ? 0.18 : 0.35);
    const theoretical =
      dte === 0
        ? {
            price:
              Math.max(callPut === "call" ? spot - strike : strike - spot, 0) +
              Math.max(0.05, spot * 0.002),
            delta:
              callPut === "call"
                ? spot >= strike
                  ? 0.8
    ...
    const netQty =
      (random() > 0.22 ? 1 : -1) *
      (1 + Math.floor(random() * (underlying === "GLD" ? 25 : 80)));
    const spreadPct = 0.01 + random() * 0.12;
    const markPrice = Math.max(
      theoretical.price,
      underlying === "GLD" ? 0.03 : 1
    );
    ...
    const quoteMinutesAgo =
      index % 29 === 0 ? 38 : index % 17 === 0 ? 7 : random() * 1.5;
    const missing = index % 37 === 0;
    const fail = index % 71 === 0 && index !== 0;
    const source = missing
      ? null
      : underlying === "GLD"
        ? "MarketData.app / OPRA mock"
        : "Bybit V5 mock";
    const dataStatus: DataStatus = fail
      ? "FAIL"
      : missing
        ? "MISSING"
        : quoteMinutesAgo > 15
    ...
    const base: RiskPosition = {
      ...
      broker:
        underlying === "GLD" ? brokers[index % brokers.length] : "SignalPlus",
      ...
      deliverableSource: fail
        ? null
        : adjusted
          ? "contract-master adjusted mock"
          : "contract-master live mock",
      ...
      modelMarkIV: missing ? null : markIV + 0.001,
      modelBidIV: missing ? null : Math.max(0.01, markIV - 0.011),
      modelAskIV: missing ? null : markIV + 0.015,
      modelIVSpread: missing ? null : 0.026,
      modelMarkIvStatus: missing ? "INVALID_INPUT" : "MODEL",
      modelBidIvStatus: missing ? "INVALID_INPUT" : "MODEL",
      modelAskIvStatus: missing ? "INVALID_INPUT" : "MODEL",
      ivReferenceSpot: missing ? null : spot,
      ivReferenceTime: missing
        ? null
        : new Date(asOf.getTime() - quoteMinutesAgo * 60_000).toISOString(),
      ivReferenceSource: missing
        ? null
        : `${source} · synchronized mock underlying`,
      unitDelta: missing
        ? null
        : Math.max(-0.75, Math.min(0.75, theoretical.delta)),
      ...
      entryCost: fail
        ? null
        : entryPrice * netQty * multiplier + Math.abs(netQty) * 0.65,
      UPL: null,
      quoteTime: missing
        ? null
        : new Date(asOf.getTime() - quoteMinutesAgo * 60_000).toISOString(),
      positionTime: new Date(
        asOf.getTime() - (1 + random() * 60) * DAY_MS
      ).toISOString(),
      source,
      dataStatus,
      ...
      brokerCutoff:
        underlying === "GLD" ? `${expiry} 15:30 ET` : `${expiry} 08:00 UTC`,
      plannedAction:
        expiryOffset <= 2 ? (random() > 0.5 ? "ROLL" : "CLOSE") : "HOLD",
  ...
  const explicit = positions
    .filter(
      position =>
        ...
        position.dataStatus !== "MISSING" &&
        position.dataStatus !== "FAIL"
    )
    .slice(0, 2);
